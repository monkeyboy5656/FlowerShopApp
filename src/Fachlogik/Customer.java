...
public class Customer extends User implements Serializable {
    ...
    public void orderCancel(Warehouse warehouse, int id) {
        for (Order o : orders) {
            if (o.getId() == id && o.getStatus().equals("Canceled")) {
                Utilities.showError("Cancel Order", "This order is already canceled");
                return;
            }
        }
        //Order a1;
        ...
        for (Order a : orders) {
            if (a.getId() == id) {
                a.setStatus("Canceled");
                Utilities.showInfo("Cancel Order", "The Order with the ID= " + a.getId() + " has been canceled");
                for (CartItem cartItem : a.cart.cartItems) {
                    Product product = cartItem.getProduct();
                    int quantity = cartItem.getQuantity();
                    warehouse.updateProduct(product, quantity);
